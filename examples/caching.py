...
@bot.command()
async def fetch_channel(ctx: commands.Context, channel: discord.TextChannel) -> None:
...
@bot.command()
async def fetch_role(ctx: commands.Context, role: discord.Role) -> None:
    role = await bot.getch_role(ctx.guild.id, role.id)
    await ctx.send(f"fetched {role.mention}!")


@bot.command()
async def fetch_guild(ctx: commands.Context, guild_id: int) -> None:
    guild = await bot.getch_guild(guild_id)
    await ctx.send(f"fetched {guild}!")


@bot.command()
async def fetch_user(ctx: commands.Context, member: discord.Member) -> None:
    user = await bot.getch_user(member.id)
    await ctx.send(f"fetched {user.mention}!")


@bot.command()
async def fetch_member(ctx: commands.Context, member: discord.Member) -> None:
